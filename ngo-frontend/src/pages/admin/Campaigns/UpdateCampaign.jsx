...
export default function UpdateCampaign() {
    ...
    const loadData = async () => {
        try {
            const res = await getAllCampaigns({ ids: [id], search: "", page_number: 1, page_size: 1 });
            if (res?.status === 1 && res.data.length) {
                ...
                setForm({
                    ...
                    other_field_json: data.other_field_json ? data.other_field_json : {}
                });

                // set pairs from JSON (if present)
                const ofj = data.other_field_json || {};
                const arr = Object.keys(ofj).length ? Object.keys(ofj).map((k) => ({ key: k, value: ofj[k] })) : [{ key: "", value: "" }];
                setPairs(arr);

                setPreview({ url: `${BASE_MEDIA_URL}${data.video_or_file || data.thumbnail}` });
    ...
    const handleMediaChange = (selectedFile) => {
        ...
        setPreview({ url: URL.createObjectURL(selectedFile), type: detectFileType(selectedFile.name).toLowerCase() });
    };

    const addPair = () => setPairs((p) => [...p, { key: "", value: "" }]);
    const removePair = (i) => setPairs((p) => p.filter((_, idx) => idx !== i));
    const updatePair = (i, field, val) => setPairs((p) => p.map((row, idx) => (idx === i ? { ...row, [field]: val } : row)));
    ...
    const handleSubmit = async (e) => {
        ...
        // build object
        const obj = {};
        pairs.forEach((r) => { if (r.key.trim()) obj[r.key] = r.value; });
        ...
        try {
            ...
            // append all fields except other_field_json (we append JSON next)
